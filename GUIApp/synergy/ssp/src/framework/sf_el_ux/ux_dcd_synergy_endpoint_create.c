...
extern uint32_t __usb_interface_descriptor_start_fs;
/* Generated by linker. */
/*LDRA_INSPECTED 219 S Linker sections start with underscore. */
...
extern void ux_device_remove_compiler_padding(UCHAR * p_device_framework, UINT length);
#pragma section = ".usb_interface_desc_fs"
#endif
...
UINT  ux_dcd_synergy_endpoint_create(UX_DCD_SYNERGY *dcd_synergy, UX_SLAVE_ENDPOINT *endpoint)
{
    ...
    UCHAR             * p_start_interface;
    UCHAR             * p_end_interface;
    UCHAR             * p_temp_interface;
    INT                 size=0;
#endif
...
#if defined(__GNUC__)
p_start_interface = (UCHAR *)&__usb_interface_descriptor_start_fs;
p_end_interface   = (UCHAR *)& __usb_descriptor_end_fs;
size = p_end_interface - p_start_interface;
p_temp_interface= p_start_interface;

#elif defined(__ICCARM__)
UCHAR   buffer[UX_SLAVE_ENDPOINT_DEFAULT_BUFFER_SIZE];
p_start_interface = (UCHAR *) (__section_begin (".usb_interface_desc_fs"));
p_end_interface = (UCHAR *) (__section_end (".usb_interface_desc_fs"));
size = p_end_interface - p_start_interface;
memset (buffer, 0, sizeof(buffer));
p_temp_interface = (uint8_t *) buffer;
memcpy (p_temp_interface, p_start_interface, size);
ux_device_remove_compiler_padding(p_temp_interface, size);
p_start_interface = p_temp_interface;
#endif

/* Scan the interface class supported by the USB device.  */
while( (p_temp_interface < (p_start_interface + size)) && (count_interface < (ULONG)UX_SLAVE_ENDPOINT_DEFAULT_BUFFER_SIZE) )
{
    if( UX_INTERFACE_DESCRIPTOR_ITEM == (*(p_temp_interface + UX_DEVICE_DESCRIPTOR_ITEM)) )
    {
        /* Check the MSC interface supported by the USB device.  */
        if(UX_DCD_SYNERGY_MSC_INTERFACE == (*(p_temp_interface + UX_ENDPOINT_DESCRIPTOR_ITEM )))
        {
            msc_flag = (ULONG)UX_DCD_SYNERGY_MSC_INTERFACE;
            break;
        }
        /* Check the CDC interface supported by the USB device.  */
        else  if ( UX_DCD_SYNERGY_CDC_INTERFACE == (*(p_temp_interface + UX_ENDPOINT_DESCRIPTOR_ITEM )) )
        {
            cdc_flag = (ULONG)UX_DCD_SYNERGY_CDC_INTERFACE;
        }
        else
        {
            hid_flag= *(p_temp_interface + UX_ENDPOINT_DESCRIPTOR_ITEM );
        }

        p_temp_interface = p_temp_interface + *p_temp_interface;
    }
