...
/*******************************************************************************************************************//**
 * @brief  Subroutine to set up a color palette for drawing glyph
 * @param  dave[in]            Pointer to dave device structure
 * @param  font_format[in]     Font format indicating number of bits per pixel
 **********************************************************************************************************************/
VOID gx_dave2d_glyph_palette_set(d2_device *dave, INT font_format)
{
    switch(font_format)
    {
        case GX_FONT_FORMAT_1BPP:
            if(1 != g_dave2d.font_bits)
            {
                /** If font format is 1 bpp, set up a 2-entry palette for drawing glyph */
        ...
        case GX_FONT_FORMAT_4BPP:
            if(4 != g_dave2d.font_bits)
            {
                /** If font format is 4bpp, setup a 16 entry palette for drawing glyph */
...
/*******************************************************************************************************************//**
 * @brief  Subroutine to set up a pixelmap palette
 * @param  dave[in]            Pointer to dave device structure
 * @param  map[in]             Pointer to GUIX pixelmap
 **********************************************************************************************************************/
VOID gx_dave2d_pixelmap_palette_set(d2_device *dave, GX_PIXELMAP *map)
{
    /** Set up pixelmap palette */
...
d2_u32 gx_dave2d_format_set(GX_PIXELMAP * map)
{
    ...
    /** Assign D/AVE 2D color format according to GUIX color format */
...
VOID _gx_dave2d_drawing_initiate(GX_DISPLAY *display, GX_CANVAS  *canvas)
{
    ...
    /** Assign x and y resolution based on the rotation angle set */
    if((0 == (INT) (display->gx_display_rotation_angle)) || (180 == (INT) (display->gx_display_rotation_angle)))
    {
 ...
 * This function is called by GUIX to draw a aliased wide line.
 * @param   context[in]         Pointer to a GUIX draw context
...
VOID _gx_dave2d_buffer_toggle(GX_CANVAS  *canvas, GX_RECTANGLE * dirty)
{
    ...
    rotation_angle = (INT) (display->gx_display_rotation_angle);
    ...
    /** Copies content of Canvas buffer to working buffer based on Color format. */
    if (180 == rotation_angle)
    {
...
VOID _gx_synergy_jpeg_draw (GX_DRAW_CONTEXT *p_context, INT x, INT y, GX_PIXELMAP * p_pixelmap)
{
    ...
    if (GX_NULL == param.output_buffer)
    {
                                                                                ...
                                                                                const GX_GLYPH *glyph, d2_u32 mode)
{
    ...
    /** Set up a color palette for 4BPP glyph */
...
#if 1
    /* set up a color palette for 1BPP glyph */
                                                                    ...
                                                                    GX_PIXELMAP *pixelmap, INT angle, INT cx, INT cy)
{
    ...
    cosv = (INT) (_gx_utility_math_cos(GX_FIXED_VAL_MAKE(angle)));
    /*LDRA_INSPECTED 50 S *//*LDRA_INSPECTED 90 S */
    sinv = (INT) (_gx_utility_math_sin(GX_FIXED_VAL_MAKE(angle)));
 ...
 * @param   p_jpeg_decode_instance[in,out]     Pointer to a JPEG Decode driver instance
...
static UINT gx_synergy_jpeg_draw_minimum_height_get (jpeg_decode_color_space_t format, GX_VALUE width, GX_VALUE height)
{
    ...
    /** Compute the decoding width and height based on pixel format. Return an error if the input JPEG size is not
                                                                                                       * valid. */
    switch (format)
    {
...
static INT gx_synergy_jpeg_draw_output_streaming_wait(sf_jpeg_decode_instance_t * p_sf_jpeg_decode)
{
    ...
    while (0U == ((UINT)(JPEG_DECODE_STATUS_OUTPUT_PAUSE) & (UINT)jpeg_decode_status))
    {
...
static VOID gx_synergy_jpeg_draw_output_streaming (jpeg_output_streaming_param_t * p_param)
{
    ...
    while(remaining_lines > 0U)
    {
        ...
        if((SSP_SUCCESS != ret) || (0U == lines_decoded))
        {
            /* Nothing to draw. Just return. */
            return;
        }
        ...
        out_pixelmap.gx_pixelmap_data_size = (ULONG)(lines_decoded * ((UINT)p_param->image_width *
                                                                       p_param->bytes_per_pixel));
        view = p_context->gx_draw_context_view_head;
