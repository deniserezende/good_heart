...
static ssp_err_t sf_el_gx_open_param_check (sf_el_gx_instance_ctrl_t * const p_ctrl,
...
void sf_el_gx_frame_toggle(ULONG display_handle, GX_BYTE ** pp_visible_frame)
{
    ...
    uint32_t  line_descending_base_address = 0U;
    ...
    /** If Line descending is enabled, set the address of last line of frame as base address. Otherwise, pass the
     * actual address of frame buffer as base address */
    if (p_ctrl->p_display_runtime_cfg->input.line_descending_enable)
    {
        line_descending_base_address = (uint32_t ) p_ctrl->p_framebuffer_write;
        line_descending_base_address = (line_descending_base_address
                                        + ((p_ctrl->bytes_per_pixel)
                                        * (p_ctrl->p_display_runtime_cfg->input.hstride)
                                        * (p_ctrl->p_display_runtime_cfg->input.vsize - 1U)));
        p_ctrl->p_display_runtime_cfg->input.p_base = (uint32_t  *)line_descending_base_address;
    }
    else
    {
...
static void sf_el_gx_canvas_clear (GX_DISPLAY * p_display, sf_el_gx_instance_ctrl_t * const p_ctrl)
{
    ...
    case GX_COLOR_FORMAT_4444ARGB:   ///< ARGB4444, 16 bits
        ...
        p_ctrl->bytes_per_pixel = 2U; ///< 2 bytes per pixel
        break;
    ...
    case GX_COLOR_FORMAT_32ARGB:  ///< ARGB8888, 32 bits
        divisor = 1;
        p_ctrl->bytes_per_pixel = 4U; ///< 4 bytes per pixel
        break;

    default:    /* Apply this divisor value for 8-bit Palette. */
        divisor = 4;
        p_ctrl->bytes_per_pixel = 1U; ///< 1 byte per pixel
        break;
    }
